...
export default function JobsPage() {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/jobs/match", {
        ...
        body: JSON.stringify({ skills: ["Python", "React", "JavaScript", "SQL"], experience_level: "Junior", target_role: "Software Engineer" }),
      });
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch("/api/jobs/upload-csv", { method: "POST", body: formData });
      const data = await res.json();
      setCsvResult(data.success ? `✅ ${data.jobs_added} jobs added` : `❌ ${data.error}`);
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setCsvResult(data.success ? `✅ ${data.seeded} demo jobs added` : data.message || "Jobs already exist");
      fetchMatches();
    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fa", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 48px", borderBottom: "1px solid #d3d7dc", background: "rgba(248,249,250,0.9)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 12, background: "#fff", border: "1px solid #d3d7dc", cursor: "pointer", fontSize: 13, fontWeight: 500, color: "#1a1c1e" }}>
            <Upload size={14} /> {csvUploading ? "Uploading..." : "Upload CSV"}
            <input type="file" accept=".csv" onChange={handleCSVUpload} style={{ display: "none" }} />
          </label>
          <button onClick={handleSeed} disabled={seeding} style={{ padding: "10px 20px", borderRadius: 12, background: "#d4af37", color: "#fff", border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 48px 100px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 42, fontWeight: 600, color: "#1a1c1e", letterSpacing: "-0.02em", marginBottom: 8 }}>
            Job <span style={{ color: "#d4af37" }}>Matches</span>
          </h1>
        ...
        <div style={{ marginBottom: 32 }}>
          <button onClick={() => setShowFilters(!showFilters)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", fontSize: 14, color: "#687078", fontWeight: 500, cursor: "pointer", marginBottom: 12 }}>
            <Search size={14} /> Filters <ChevronDown size={14} style={{ transform: showFilters ? "rotate(180deg)" : "none", transition: "0.2s" }} />
          </button>
          <AnimatePresence>
            {showFilters && (
        ...
        {loading && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 80, gap: 12, color: "#687078" }}>
            <Loader2 size={20} className="animate-spin" /> Finding your best job matches...
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {filtered.length === 0 && (
              <div style={{ textAlign: "center", padding: 60, color: "#9ea5ad" }}>
                ...
                <p style={{ fontSize: 16, fontWeight: 500 }}>No jobs found. Upload a CSV or seed demo jobs to get started.</p>
              </div>
            )}
            {filtered.map((match, i) => (
              <motion.div key={match.job_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                style={{ background: "#fff", borderRadius: 16, border: "1px solid #d3d7dc", padding: "24px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 24, boxShadow: "0 2px 8px rgba(0,0,0,0.02)", transition: "box-shadow 0.2s, border-color 0.2s" }}
              ...
              >
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                    <h3 style={{ fontSize: 18, fontWeight: 600, color: "#1a1c1e" }}>{match.job.job_title}</h3>
                  ...
                  <p style={{ fontSize: 13, color: "#9ea5ad", lineHeight: 1.5, maxWidth: 600 }}>{match.job.job_description?.slice(0, 120)}...</p>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 12 }}>
                    {(match.job.required_skills || []).slice(0, 5).map(skill => (
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, minWidth: 120 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {match.job.job_link && (
                      <a href={match.job.job_link} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 4, padding: "8px 16px", borderRadius: 10, background: "#d4af37", color: "#fff", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
