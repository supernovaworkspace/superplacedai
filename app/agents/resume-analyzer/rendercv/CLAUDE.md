...
## What is Superplaced AI CV?

Superplaced AI CV is a resume/CV builder for academics and engineers. Users write CVs in YAML, and Superplaced AI CV produces PDFs with professional typography. It supports multiple themes (classic, moderncv, sb2nov, engineeringresumes, engineeringclassic) and custom themes. Deployed at superplaced-cv.com as a web app.

## Core Pipeline

```
YAML → (ruamel.yaml) → Python dict → (pydantic) → Superplaced AI CVModel → (jinja2) → Typst file → (typst) → PDF
```

1. **Parse**: `ruamel.yaml` reads YAML into Python dicts (`schema/yaml_reader.py`)
2. **Validate**: `pydantic` validates into `Superplaced AI CVModel` (`schema/models/superplaced-cv_model.py`)
...
src/superplaced-cv/
  cli/                          # Typer CLI (render, new, create-theme commands)
    render_command/run_superplaced-cv.py  # Main render orchestration
  schema/                       # Data models and validation
    models/
      ...
      superplaced-cv_model.py         # Top-level Superplaced AI CVModel
    superplaced-cv_model_builder.py   # Builds Superplaced AI CVModel from YAML with overrides
...
Tests mirror the source structure: `src/superplaced-cv/renderer/templater/date.py` → `tests/renderer/templater/test_date.py`.
...
- `minimal_superplaced-cv_model` / `full_superplaced-cv_model` — pre-built models for testing
