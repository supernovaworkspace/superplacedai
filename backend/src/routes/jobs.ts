...
import { callEmbeddings, callAI } from "../lib/gemini";
...
router.post("/match", async (req, res) => {
  try {
    ...
    if (!user_id || !resume_id) return res.status(400).json({ success: false, error: "user_id and resume_id are required" });
    ...
    const resumeEmbedding = await callEmbeddings(JSON.stringify(resumeSkills));

    const matchRecords = [];
    for (const job of jobs) {
      const overlap = job.required_skills.filter((skill: string) => resumeSkills.includes(skill)).length;
      const keywordScore = Math.round((overlap / Math.max(job.required_skills.length, 1)) * 100);
      const jobEmbedding = await callEmbeddings(job.job_description || "");
    ...
    const { error: insertError } = await supabase.from("job_matches").insert(insertData);
    if (insertError) throw insertError;

    await supabase.from("activity_logs").insert([{ user_id, action: "job_match", metadata: { resume_id }, created_at: new Date().toISOString() }]);
